<gb-paging>
  <div class="gb-paging { opts.style() }">
    ...
    <ul class="gb-paging__pages" if={ showPages }>
      <span class="gb-paging__ellipsis" if={ showLowOverflow() }>&hellip;</span>
      <li each={ page in pages() }>
        <a class="gb-paging__page { currentPage() + 1 === page ? 'selected' : '' }" onclick={ jumpTo }>{ page }</a>
      </li>
      <span class="gb-paging__ellipsis" if={ showHighOverflow() }>&hellip;</span>
    </ul>
  ...
  <script>
    ...
    this.pages = () => opts.flux.page.displayPages(limit);
    this.showLowOverflow = () => this.pages()[0] !== 1;
    this.showHighOverflow = () => this.pages().pop() !== opts.flux.page.total + 1;
